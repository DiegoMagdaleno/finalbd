...
const ProductCard = ({ product }) => (
  <div className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 group">
    ...
    <div className="p-4">
      ...
      <p className="text-sm text-gray-600 mt-1 h-10">{product.description || 'Sin descripción.'}</p>
      <div className="mt-4 flex justify-between items-center">
        <span className="text-2xl font-bold text-koppel-blue">${parseFloat(product.price).toFixed(2)}</span>
        <button className="px-4 py-2 bg-koppel-blue text-white text-sm font-bold rounded-full hover:bg-blue-800 transition-transform transform hover:scale-105">
          Agregar
        </button>
      </div>
    </div>
